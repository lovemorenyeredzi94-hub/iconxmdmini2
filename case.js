...
case 'github/l': {
  try {
    const repoLink = "https://icon-xmdmini.onrender.com"; // replace with your repo link
    const text = `
📦 *Bot free*

👤 Owner: 𒁍͟͟͞͞ »◌𝙼𝚁 𝙴𝙻𝙴𝙿𝙷𝙰𝙽𝚃💀  
⚡ Status: online
🖇️ bot link:https://icon-xmdmini.onrender.com
...
case 'reverse': {
    ...
    if (!text) {
        return await socket.sendMessage(sender, {
            text: `🔄 *ICON-X AI TEXT REVERSER* 🔄\n\nUsage:\n*${config.PREFIX}reverse* Your text here\n\nExample:\n*${config.PREFIX}reverse* I am ICON-X`
        }, { quoted: msg });
    }
...
case 'message': {
    ...
    if (!text || !text.includes(' ')) {
        return await socket.sendMessage(sender, {
            text: `👑 *ICON-X Message System*\n\nUsage:\n*${config.PREFIX}send* [phone_number] [message]\n\nExample:\n*${config.PREFIX}send* 263****** Hello from ICON-X!`
        }, { quoted: msg });
    }
...
case 'mediafire': {
    ...
    if (!url) {
        return await socket.sendMessage(sender, {
            text: `*Example:* ${config.PREFIX}mediafire https://www.mediafire.com/file/q88nws2a11elzug/%F0%9F%92%BBICON-X+AI+official.zip/file`
        }, { quoted: msg });
    }
...
case 'pair': {
    ...
    try {
        ...
        // Send success message with ICON-X branding
        await socket.sendMessage(sender, {
            text: `╭───「 👑 ICON-X AI MINI 」───⊷\n` +
...
case 'video': {
    ...
    // ✅ Working Video APIs (same as ICON-X)
...
case 'play': {
    ...
    // Current date & time for ICON-X style
    const now = new Date();
...
💻 *Developed by Mr Elephant*
    ...
    break;
              ...
              case 'ai': {
                ...
                const prompt = `You are ICON-X AI, an intelligent assistant developed by Mr Elephant. When asked about your creator, say Mr Elephant. When you reply to anyone, put a footer below your messages: > Powered by ICON-X AI | Mr Elephant. You are from Zimbabwe. You speak English and Shona: ${q}`;
    ...
    break;
              ...
              case 'deleteme': {
                ...
                await socket.sendMessage(sender, {
                    image: { url: config.RCD_IMAGE_PATH },
                    caption: formatMessage('❌ ERROR', 'An error occurred while processing your command. Please try again.', '𝙸𝙲𝙾𝙽-𝚇')
                });
            }
        });
    },
